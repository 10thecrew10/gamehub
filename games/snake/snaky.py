...
def run_snake(game_id: int, user_id: int):
    ...
    while True:
        points = runGame()
        showGameOverScreen(game_id, user_id, points)
...
def showGameOverScreen(game_id: int, user_id: int, points: int):
    gameOverFont = pygame.font.Font('freesansbold.ttf', 50)
    highest_score = get_score(user_id, game_id)
    gameSurf, overSurf = None, None
    if highest_score < points:
        update_score(user_id, game_id, points)
        gameSurf = gameOverFont.render(f'New highest score!', True, WHITE)
        overSurf = gameOverFont.render(str(points), True, WHITE)
    else:
        gameSurf = gameOverFont.render('Game over!', True, WHITE)
        overSurf = gameOverFont.render(f'Highest score {str(highest_score)}', True, WHITE)
    gameRect = gameSurf.get_rect()
    overRect = overSurf.get_rect()
    gameRect.midtop = (WINDOWWIDTH / 2, 10 + 75)
    overRect.midtop = (WINDOWWIDTH / 2, gameRect.height + 10 + 100)
